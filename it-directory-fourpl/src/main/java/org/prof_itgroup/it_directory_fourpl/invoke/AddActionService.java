...
public class AddActionService implements ActionService {
	...
	public AddActionService(PropertiesService propertiesService) {
	...
	public String action(String message, Map<String, Object> environment) throws Exception {
		...
		try {
			...
			boolean result = customsAdd(directory);
	...
	private boolean customsAdd(Directory directory) throws Exception {
		boolean res = true;
		try {
			...
			if (!loginError) {
				try {
					...
					JsonNode fieldsNode = mapper.valueToTree(fieldsMap);

					mainNode.set("doc", fieldsNode);

					String insertRequest = mapper.writeValueAsString(mainNode);

					HttpHeaders headers = new HttpHeaders();
					headers.setContentType(MediaType.APPLICATION_JSON);
					HttpEntity<String> entity = new HttpEntity<String>(insertRequest, headers);

					String insertResponseBody = restTemplate.postForObject("https://api.scorocode.ru/api/v1/data/insert", entity, String.class);
					Map<String, Object> insertResponseMap = new HashMap<String, Object>();

					insertResponseMap = mapper.readValue(insertResponseBody, new TypeReference<HashMap<String, Object>>() {
					});
					res = (Boolean) insertResponseMap.get("error");
					if (res) {
						log.info("ItMessageListener error:" + insertResponseBody);
					} 
				} catch (Exception e) {
