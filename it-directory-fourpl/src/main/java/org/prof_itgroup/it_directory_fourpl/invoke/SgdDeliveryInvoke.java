...
public class SgdDeliveryInvoke implements InvokeRequestInterface {
	...
	public After afterRequest(String response, String context, Map<String, Object> environment) {
		...
		try {
			...
			boolean error = Boolean.parseBoolean(response);
			if (!error) {
				log.info("Модуль it-directory-fourpl успешно выполнил операцию типа "+directory.getEnum());
				after = new FinalAfter();
			} else {
				log.info("Модуль it-directory-fourpl не смог выполнить операцию типа "+directory.getEnum()+" будет выполнена еще одна попытка");
	...
	private  After countLogic(String context) {
		...
		try {
			...
			if (count >= numberOfAttempts) {
				...
				String message = "Исчерпано количество попыток 4PL для сообщения:"+json;
				log.info("Модуль it-directory-fourpl исчерпал количество попыток для сообщения "+json);
