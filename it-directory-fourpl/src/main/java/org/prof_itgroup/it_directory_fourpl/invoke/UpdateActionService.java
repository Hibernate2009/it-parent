...
public class UpdateActionService implements ActionService{
	...
	public UpdateActionService(PropertiesService propertiesService) {
	...
	public String action(String message, Map<String, Object> environment) throws Exception {
		...
		try {
			...
			boolean	result = customsUpdate(directory);
	...
	private boolean customsUpdate(Directory directory) throws Exception {
		boolean res = true;
		try {
			...
			if (!loginError) {
				try {
					...
					JsonNode node = mapper.valueToTree(fieldsMap);
					ObjectNode setNode = mapper.createObjectNode();
					setNode.set("$set", node);
					mainNode.set("doc", setNode);
					
					String updateRequest = mapper.writeValueAsString(mainNode);
					...
					res = (Boolean)updateResponseMap.get("error");
					if (res){
